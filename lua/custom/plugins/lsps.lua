...
return {
      ...
      local servers = {
        ...
        postgres_lsp = {
          on_attach = function(client, bufnr)
            vim.notify('postgres_lsp running with DATABASE_URL=' .. (vim.env.DATABASE_URL or 'nil'))
          end,
        },
  ...
  -- 2) Trouble: fast, persistent lists (refs/diagnostics/etc.)
  {
    'folke/trouble.nvim',
    cmd = { 'Trouble' },
    opts = {},
  ...
  -- 3) Peek instead of jump when you want context
  {
    'dnlhc/glance.nvim',
    event = 'LspAttach',
    config = true,
    keys = {
      ...
      { 'gpD', '<cmd>Glance declarations<cr>', desc = 'Peek Declaration' },
    },
  },

  -- 4) Inline rename (more fluid than the rename prompt)
  {
  ...
  -- 5) Code actions with live preview of edits
  {
